...
function mapCategory(vexCat) {
    ...
    if (c.includes('youtube') || c.includes('yt')) {
        if (c.includes('подписчики'))
            return { platform: 'YOUTUBE', category: 'SUBSCRIBERS', targetType: 'CHANNEL' };
        if (c.includes('просмотр'))
            return { platform: 'YOUTUBE', category: 'VIEWS', targetType: 'VIDEO' };
        if (c.includes('лайк'))
            return { platform: 'YOUTUBE', category: 'LIKES', targetType: 'VIDEO' };
    }
    return null; // Ignore unsupported categories to keep it clean
}
// Format Name with Tiers
function formatServiceName(name, category, priceDetails) {
    // Determine tier based on explicit keywords or base price proxy
    let tier = '';
    const n = name.toLowerCase();
    if (n.includes('жив'))
        tier = '[Живые] ';
    else if (n.includes('премиум') || n.includes('premium') || n.includes('hq') || n.includes('выс'))
        tier = '[Премиум] ';
    else if (n.includes('эконом') || n.includes('дешев') || n.includes('бот'))
        tier = '[Эконом] ';
    else
        tier = '[Стандарт] ';
...
async function main() {
    ...
    const catCache = {};
    async function getCategoryId(platform, category, targetType) {
    ...
    if (!res.ok)
        throw new Error(`Vexboost API error: ${res.statusText}`);
    ...
    for (const pool of [hqServices, otherServices, lqServices]) {
        for (const s of pool) {
            if (selectedServices.length >= TARGET_TOTAL_SERVICES)
                break;
