...
export class SmartAnalyzerService {
    ...
    static async getLinkTypes() {
        // @ts-ignore - LinkType might not be in generated client yet
    ...
    static async analyzeService(name: string, description: string = '', categoryInput: string = ''): Promise<AnalyzedService | null> {
        ...
        const result = SmartAnalyzerLogic.detectSync(name, description, categoryInput, platforms, linkTypes);

        // Alert if platform is recognized but targetType is generic
        if (result.platform !== 'OTHER' && (result.targetType === 'POST' || result.targetType === 'ALL')) {
            await NotificationService.notifyAnalyzerFailure({
                serviceName: name,
                platform: result.platform,
                category: result.category,
                suggestedTarget: result.targetType
            });
        }
    ...
    static async importSingle(
        ...
        const linkTypes = await this.getLinkTypes();
        const analysis = SmartAnalyzerLogic.detectSync(rawSvc.name, rawSvc.description || '', rawSvc.category || '', platforms, linkTypes);

        // Alert if platform is recognized but targetType is generic
        if (analysis.platform !== 'OTHER' && (analysis.targetType === 'POST' || analysis.targetType === 'ALL')) {
            await NotificationService.notifyAnalyzerFailure({
                serviceName: rawSvc.name,
                platform: analysis.platform,
                category: analysis.category,
                suggestedTarget: analysis.targetType
            });
        }
