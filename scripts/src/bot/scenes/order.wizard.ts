...
async function showFinalConfirmation(ctx: any) {
  ...
  if (ctx.wizard.state.orderData.warning) {
    summaryText += `\n\n⚠️ <b>ОБРАТИТЕ ВНИМАНИЕ:</b>\n<i>${ctx.wizard.state.orderData.warning}</i>`;
  }
...
export const orderWizard = new Scenes.WizardScene(
  ...
  async (ctx: any) => {
    ...
    if (ctx.wizard.state.orderData?.service) {
      ...
      const validation = await LinkService.validate(link, service.platform, service.targetType);
    ...
    await ctx.reply(`🔍 Платформа: <b>${platformName}</b>\nТип: <b>${analysis.targetType}</b>\nВыберите категорию:`, {
      parse_mode: 'HTML',
...
orderWizard.action(/^select_cat_(.+)$/, async (ctx: any) => {
  ...
  await ctx.editMessageText(`📦 Категория: <b>${catName}</b>\nВыберите подходящий тариф:`, {
    parse_mode: 'HTML',
...
orderWizard.action(/^select_group_(.+)$/, async (ctx: any) => {
  ...
  await ctx.editMessageText(`📁 Группа: <b>${groupName}</b>\nВыберите подходящий тариф:`, {
    parse_mode: 'HTML',
    ...Markup.inlineKeyboard([...buttons, [Markup.button.callback('🔙 Назад', 'back_to_cats')]])
  });
});
...
orderWizard.action('back_to_cats', async (ctx: any) => {
  ...
  const buttons = categories.map(c => [Markup.button.callback(c.name, `select_cat_${c.id}`)]);
  await ctx.editMessageText(`🔍 Платформа: <b>${platformName}</b>\nТип: <b>${targetType}</b>\nВыберите категорию:`, {
    parse_mode: 'HTML',
    ...Markup.inlineKeyboard(buttons)
  });
  await ctx.answerCbQuery();
