...
import { ShoppingBag, Trash2, ArrowRight, Package, Zap } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import Link from 'next/link';
import { getStrategyServices } from '@/app/_actions/services/getStrategyServices';

/** Typed cart item for production safety */
interface CartItem {
    id: string;
    type: 'GROWTH_PACKAGE' | 'SINGLE_SERVICE';
    serviceId?: string;
    platform?: string;
...
export default function CartPage() {
    ...
    const handleCheckout = async () => {
        ...
        try {
            // 0. RESET AUTH MODE IF EMAIL CHANGED
            // (Wait, we'll let the user change it manually if they want)
            ...
            const batchItems: { serviceId: string; link: string; quantity: number }[] = [];
                            ...
                            <div className="bg-slate-900 p-8 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden">
                                ...
                                <div className="relative z-10">
                                    ...
                                    <div className="space-y-4 mb-8">
                                        ...
                                        {/* Discount line — only show when a discount applies */}
                                    ...
                                    <div className="space-y-4 mb-6">
                                        <div className="space-y-2">
                                            ...
                                            <input 
                                                ...
                                                className={`w-full bg-white/5 border rounded-xl py-3 px-4 text-sm font-medium outline-none transition-all text-white placeholder:text-slate-600 ${
                                                    email && !isEmailValid ? 'border-rose-500/50 focus:border-rose-500' : 'border-white/10 focus:border-blue-500'
                                                }`}
                                            />
                                            {email && !isEmailValid && (
                                                <p className="text-[9px] font-bold text-rose-400 ml-1">Некорректный формат email</p>
                                            )}
                                        </div>
