...
import { Settings, Layers, Globe, Shield, Mail, FlaskConical } from 'lucide-react';
...
export default async function SettingsPage(props: { searchParams: Promise<any> }) {
  ...
  smtpConfig.password = smtpConfig.password ? '********' : ''; 

  const tabs = [
    { label: 'Настройки Проекта', icon: <Settings size={16} />, id: 'config', description: 'Конфигурация текущего проекта (бренд, платежи, телеграм)' },
    { label: 'Интеграции', icon: <Globe size={16} />, id: 'platforms', description: 'Управление социальными сетями и парсингом ссылок' },
    isGlobalAdmin && { label: 'Глобальная Платформа', icon: <Shield size={16} />, id: 'global-settings', description: 'Системные лимиты, маржа и конфигурация ядра платформы' },
    isGlobalAdmin && { label: 'Экстренная Почта (SMTP)', icon: <Mail size={16} />, id: 'system-smtp', description: 'Настройки системного почтового сервера' },
    isGlobalAdmin && { label: 'Песочница', icon: <FlaskConical size={16} />, id: 'sandbox', description: 'Режим тестирования с мок-провайдерами и тестовыми платежами' },
    ...
    <div className="space-y-8 pb-12">
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-black text-slate-800 tracking-tight">Настройки и Интеграции</h1>
            <p className="text-xs text-slate-500 font-bold uppercase tracking-wider">Управление платформой и проектами</p>
          </div>
        </div>
      </div>
      <AdminTabs tabs={tabs}>
        ...
        {isGlobalAdmin && (
          <div>
            <SmtpSettingsForm initialConfig={smtpConfig} />
          </div>
        )}
