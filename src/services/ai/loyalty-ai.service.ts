...
export type UserSegment = 'NEWCOMER' | 'SILENT' | 'POWER_USER' | 'ADVOCATE';

export interface AIInsight {
    type: 'SEGMENT' | 'NPS' | 'REFERRAL';
    title: string;
    message: string;
...
export class LoyaltyAIService {
    ...
    static async getUserAnalysis(userId: string) {
        ...
        const insights = this.generateInsights(user, segment);
        const recommendations = this.generateRecommendations(user, segment);
    ...
    private static determineSegment(user: any): UserSegment {
        const now = new Date();
    ...
    private static generateInsights(user: any, segment: UserSegment): AIInsight[] {
        ...
        insights.push({
            ...
            level: segment === 'POWER_USER' ? 'SUCCESS' : 'INFO'
        });
    ...
    private static generateRecommendations(user: any, segment: UserSegment): AIRecommendation[] {
