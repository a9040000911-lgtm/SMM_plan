    ...
    static async processPendingOrders(specificOrderId?: number) {
        ...
        for (const o of pending) {
            try {
                ...
                if (isNetworkError) {
                    // КРИТИЧЕСКИЙ СЛУЧАЙ: Таймаут API. Заказ мог создаться, а мог и нет.
                    // МЫ НЕ ДЕЛАЕМ ВОЗВРАТ. Помечаем заказ как "Зависший" для ручной или автоматической проверки.
                    await prisma.order.update({
                        where: { id: o.id },
                        data: {
                            status: 'PROCESSING', // Оставляем в PROCESSING, чтобы не вернулись деньги
                            metadata: {
                                ...(o.metadata as any || {}),
