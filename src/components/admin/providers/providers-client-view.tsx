...
import { SecurityReportModal } from '@/components/admin/providers/security-report-modal';
import { AdminTableCard } from '@/components/admin/core/admin-table-card';
import { useRouter } from 'next/navigation';

export function ProvidersClientView({
  initialProviders,
  initialFinancials,
  ...
  const activeProjectId = initialActiveProjectId;
    ...
    try {
      ...
      if (formData.id) {
        res = await updateProviderAction(formData.id, {
          ...
          verificationCode: verificationCode // Pass 2fa code if present
        });
      } else {
  ...
  const handle2FASubmit = async (e: React.FormEvent) => {
    ...
    // Trigger handleSave with the synthetic event but using state formDataCache
  ...
  const handlePaymentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        router.refresh(); // Refresh balances
      } else {
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100">
                  {financials.forecasts.map((f: any) => (
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {providers.map((provider) => {
                const forecast = financials?.forecasts.find((f: any) => f.providerName === provider.name);
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl p-6 space-y-6">
            ...
            <form onSubmit={handle2FASubmit} className="space-y-4">
              ...
              >
                {isSaving ? 'Проверка...' : 'Подтвердить и Сохранить'}
              </button>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] w-full max-w-2xl shadow-2xl animate-in fade-in zoom-in duration-200 overflow-hidden">
            ...
            <div className="p-8 space-y-6 overflow-y-auto max-h-[70vh]">
              ...
              {financials?.forecasts.find((f: any) => f.providerName === selectedProvider.name) && (
                <div className="space-y-3">
                  ...
                  <div className="p-4 bg-indigo-50 rounded-2xl border border-indigo-100">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-xs font-black text-indigo-600">{financials.forecasts.find((f: any) => f.providerName === selectedProvider.name)?.ml.confidence}%</span>
                    </div>
                    <div className="w-full h-1.5 bg-indigo-200 rounded-full overflow-hidden mb-3">
                      <div className="h-full bg-indigo-600 rounded-full" style={{ width: `${financials.forecasts.find((f: any) => f.providerName === selectedProvider.name)?.ml.confidence}%` }}></div>
                    </div>

                    <div className="space-y-1">
                      {financials.forecasts.find((f: any) => f.providerName === selectedProvider.name)?.ml.details?.map((detail: string, i: number) => (
                      ...
                      {(!financials.forecasts.find((f: any) => f.providerName === selectedProvider.name)?.ml.details) && (
