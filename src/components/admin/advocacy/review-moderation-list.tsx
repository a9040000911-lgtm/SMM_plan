    ...
    async function handleModeration(reviewId: string, status: 'APPROVED' | 'REJECTED', qualityScore: number = 2) {
        ...
        try {
            const res = await fetch('/api/admin/advocacy/reviews', {
                ...
                body: JSON.stringify({ reviewId, action: status === 'APPROVED' ? 'approve' : 'reject', qualityScore: status === 'APPROVED' ? (qualityScore === 1 ? 'LOW' : qualityScore === 3 ? 'HIGH' : 'MEDIUM') : undefined })
            });
                        ...
                        <div className="space-y-6">
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-2xl bg-indigo-600 flex items-center justify-center text-white font-black text-xs uppercase">
                                    {review.username?.substring(0, 2) || 'US'}
                                </div>
                                <div className="flex flex-col overflow-hidden">
                                    <span className="text-sm font-black text-slate-900 truncate">@{review.username || 'user'}</span>
                    ...
                    <div className="p-8 flex-1 flex flex-col justify-between">
                        ...
                        <div className="mt-8 flex flex-wrap items-center justify-end gap-3">
                            <button
                                ...
                                disabled={!!processing}
                                className="px-6 py-2.5 rounded-xl border-2 border-slate-100 text-slate-400 hover:text-rose-500 hover:border-rose-100 transition-all text-[10px] font-black uppercase tracking-widest flex items-center gap-2 disabled:opacity-50"
                            >
                                {processing === review.id ? <Loader2 size={14} className="animate-spin" /> : <X size={14} />}
                                Отклонить
                            </button>
                            <button
                                onClick={() => handleModeration(review.id, 'APPROVED')}
                                disabled={!!processing}
                                className="px-8 py-2.5 rounded-xl bg-indigo-600 text-white hover:bg-black transition-all text-[10px] font-black uppercase tracking-widest flex items-center gap-2 shadow-[0_10px_20px_rgba(79,70,229,0.2)] disabled:opacity-50"
                            >
                                {processing === review.id ? <Loader2 size={14} className="animate-spin" /> : <Check size={14} />}
                                Одобрить и наградить
                            </button>
                        </div>
                    </div>
                </div>
