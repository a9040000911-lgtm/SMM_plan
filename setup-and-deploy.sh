...
# --- SMTP НАСТРОЙКИ ---
echo "📧 Настройка SMTP (для отправки чеков и сброса пароля)..."
if [ -z "$SMTP_USER" ]; then read -p "Введите SMTP User (Email): " SMTP_USER; fi
if [ -z "$SMTP_PASS" ]; then read -p "Введите SMTP Password (Пароль приложения): " SMTP_PASS; fi
if [ -z "$SMTP_HOST" ]; then read -p "Введите SMTP Host (по умолчанию smtp.yandex.ru): " SMTP_HOST; fi
if [ -z "$SMTP_PORT" ]; then read -p "Введите SMTP Port (по умолчанию 465): " SMTP_PORT; fi
