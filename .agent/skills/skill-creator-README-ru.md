...
```bash
python scripts/init_skill.py <имя> --path <директория> [опции]

Обязательные аргументы:
  имя                   Имя навыка (kebab-case, например "pdf-processor")
  --path ПУТЬ           Директория для создания

Опции:
  --resources СПИСОК    Создать директории (scripts,references,assets)
  --examples            Создать примеры тестов
  --interface КЛЮЧ=ЗНАЧ UI метаданные (display_name, short_description, etc.)
...
```bash
python scripts/quick_validate.py <путь-к-навыку> [опции]
...
```bash
python scripts/generate_openai_yaml.py <путь-к-навыку> [опции]

Опции:
  --interface КЛЮЧ=ЗНАЧ Установить поле
  --tags ТЕГИ           Теги через запятую
...
```bash
python scripts/package_skill.py <путь-к-навыку> [опции]

Опции:
  --output ПУТЬ         Путь для .skill файла
...
```bash
python scripts/publish_skill.py <путь-к-навыку> [опции]

Опции:
  --api-key КЛЮЧ        API ключ (или переменная SKILLS_SH_API_KEY)
...
Примеры:
  ...
  # Публикация
  export SKILLS_SH_API_KEY="ваш-api-ключ"
...
```bash
python scripts/aggregate_benchmark.py <директория-бенчмарка> [опции]

Опции:
  --skill-name ИМЯ      Имя навыка
  --output ПУТЬ         Путь для benchmark.json
  --analytics           Генерировать отчёт с аналитикой
  --trend ДИРЕКТОРИЯ    Директория с историей для трендов
  --compare ФАЙЛ        Сравнить с предыдущим benchmark.json
...
- Тестируйте краевые случаи и ошибки
...
### Навык не активируется (не триггерится)
...
- **Документация:** директория `references/`
- **Примеры:** `references/examples.md`
- **Решение проблем:** `references/troubleshooting.md`
- **Композиция навыков:** `references/composition.md`
